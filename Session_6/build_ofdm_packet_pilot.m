function [ofdm_packet] = build_ofdm_packet_pilot(bitstream,qam_orders, pilot_symbs)
    % Amount of frequency bins
    nfft = length(qam_orders);
    
    % Amount of bits of the datastream is packed within each frame
    frame_bits = 0;
    for i=1:2:(nfft/2) % Not -1 since we include the DC frequency (which should be zero anyways)
    ...
    for bin=1:2:nfft/2 %put data only in uneven bins
        if qam_orders(bin) ~= 0
    ...
    for bin=2:2:nfft/2
        %add pilot tones in even bins
        symbs =  ones(size(ofdm_packet(bin, :))) * pilot_symbs(bin/2);
        ofdm_packet(bin,:) = symbs;
        ofdm_packet(nfft-bin+2,:) = conj(symbs);
    end
end
