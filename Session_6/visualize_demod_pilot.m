...
image_data_length  = ceil(length(image_data)/size(calc_channel_freq_responses,2));
...
for pack = 2:size(calc_channel_freq_responses,2)
    pause(send_time);
    ...
    subplot(2,2,4);
    colormap(colorMap);
    if pack == size(calc_channel_freq_responses,2)
